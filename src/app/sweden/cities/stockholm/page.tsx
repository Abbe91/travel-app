...
const Gothenburg = () => {
  ...
  const myCity = countryConfig.stads.find(
    (city: { name: string }) => city.name.toLowerCase() === stad.toLowerCase()
  )?.name!;
  return (
      ...
      <div className="relative mb-16 h-screen w-full">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <p className="text-lg text-center text-white">
            Discover the beauty and culture of {countryConfig.name}. Explore
            cities, famous places, and immerse yourself in the rich heritage of
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="text-center mb-8">
          <Link href={`/${countryName}/cities/${stad}/favorites`}>
            <p className="text-teal-500 hover:text-teal-400 text-lg underline">
              View Your Favorites
            </p>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {countryConfig.places.map(
            (place: string, index: Key | null | undefined) => (
              <Box key={index} country={countryName} stad={stad} name={place} />
            )
          )}
        </div>
      </div>
