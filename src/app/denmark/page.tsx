...
const Denmark = () => {
  ...
  // Meta information
  const metaTitle = "Explore Denmark";
  const metaContent =
    "Discover the beauty and charm of Denmark, from its vibrant cities to its stunning landscapes.";

  // Query parameters
  const countryName = "Denmark";
  const countryDescription =
    "Denmark is known for its beautiful landscapes, rich history, and vibrant culture. Explore the various attractions and experiences that this country has to offer.";
  ...
  const culturalHighlights: any[] = [
    {
      title: "Cuisine",
      description:
        "Experience the unique flavors of Denmark's cuisine, from traditional dishes to modern culinary delights.",
      image: kok.src,
    },
    {
      title: "Festivals",
    ...
    },
    {
      title: "Art and Music",
      description:
        "Discover the vibrant art and music scene in Denmark, featuring local and international talents.",
      image: kok.src,
    },
    {
      title: "Historical Sites",
      description:
        "Explore the historical sites of Denmark, each with its own unique story and significance.",
      image: kok.src,
    },
  ];
  useEffect(() => {
            ...
            {countryConfig.stads.map(
              (stad: { name: string; image: string }, index: number) => (
                ...
                >
                  ...
                  <Link href={`/denmark/cities/${stad.name.toLowerCase()}`}>
